...
public class StaffDashboard extends JFrame {
    ...
    private void initializeComponents() {
        ...
        carSearchPanel = createCarSearchPanel();
        ...
        mainContentPanel.add(carSearchPanel, "CAR_SEARCH");
    ...
    private void setupLayout() {
        ...
        JPanel backgroundPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                // Create beautiful gradient from light green to white
                ...
                g2d.setColor(new Color(46, 204, 113, 5));
        ...
        JPanel headerPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                GradientPaint gradient = new GradientPaint(
                        0, 0, ModernUITheme.SECONDARY_COLOR,
                        0, getHeight(), ModernUITheme.SUCCESS_COLOR);
                g2d.setPaint(gradient);
        ...
        welcomeLabel = new JLabel(getStaffWelcomeMessage());
    ...
    private JPanel createDashboardPanel() {
        ...
        scrollPane.getVerticalScrollBar().setUI(new javax.swing.plaf.basic.BasicScrollBarUI() {
            ...
            protected void configureScrollBarColors() {
                this.thumbColor = new Color(46, 204, 113);
    ...
    private JPanel createSidebarHeader() {
        JPanel headerPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                GradientPaint gradient = new GradientPaint(
                        ...
                        0, getHeight(), new Color(39, 174, 96)); // Darker green
                g2d.setPaint(gradient);
        ...
        JLabel sidebarHeader = new JLabel("🚀 Staff Operations");
    ...
    private JPanel createNavigationMenu() {
        ...
        // Navigation Buttons with attractive styling - Staff focused
        JButton rentalManagementBtn = createProfessionalMenuButton("📋 Rental Management",
                "Manage car rentals and bookings", new Color(52, 152, 219)); // Blue
        JButton customerManagementBtn = createProfessionalMenuButton("👤 Customer Management",
                "Manage customer information", new Color(46, 204, 113)); // Green
        JButton carSearchBtn = createProfessionalMenuButton("🔍 Search Cars",
                "Search and view available cars", new Color(155, 89, 182)); // Purple
        JButton reportsBtn = createProfessionalMenuButton("📊 View Reports",
                "View rental reports and statistics", new Color(230, 126, 34)); // Orange
    ...
    private JPanel createContentHeader() {
        ...
        JLabel title = new JLabel("📊 Staff Overview");
        title.setFont(new Font("Segoe UI", Font.BOLD, 24));
        title.setForeground(ModernUITheme.TEXT_PRIMARY);

        JLabel subtitle = new JLabel("Real-time rental data and customer insights");
    ...
    private JPanel createDataOverview() {
        ...
        JPanel rentalChart = createRentalActivityChart();
        topRow.add(rentalChart);

        // Car availability chart
        JPanel availabilityChart = createCarAvailabilityChart();
        topRow.add(availabilityChart);
        ...
        // Recent rentals
        JPanel recentRentals = createRecentRentals();
        bottomRow.add(recentRentals);

        // Staff alerts
        JPanel staffAlerts = createStaffAlerts();
        bottomRow.add(staffAlerts);
    ...
    private JPanel createRentalActivityChart() {
        JPanel chart = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2d.drawString("📋 Rental Activity", 20, 30);
                ...
                // Calculate rental data (last 6 days or available data)
                int[] data = calculateRentalData(rentals);
                int barWidth = (getWidth() - 60) / data.length;
                int maxValue = Arrays.stream(data).max().orElse(5);
                if (maxValue == 0)
                    maxValue = 5; // Prevent division by zero
    ...
    private int[] calculateRentalData(List<Rental> rentals) {
        // Calculate rentals for last 6 days
        int[] dailyRentals = new int[6];
        java.time.LocalDate now = java.time.LocalDate.now();

        for (Rental rental : rentals) {
            try {
                // Parse rental date and calculate which day it belongs to
                java.time.LocalDate rentalDate = rental.getStartDate();
                if (rentalDate != null) {
                    long daysAgo = java.time.temporal.ChronoUnit.DAYS.between(rentalDate, now);

                    if (daysAgo >= 0 && daysAgo < 6) {
                        dailyRentals[(int) daysAgo]++;
                    }
                }
            } catch (Exception e) {
                // Skip invalid dates
    ...
    private JPanel createCarAvailabilityChart() {
        JPanel chart = new JPanel() {
            @Override
            protected void paintComponent(Graphics g) {
                super.paintComponent(g);
                ...
                g2d.setColor(ModernUITheme.TEXT_PRIMARY);
                g2d.setFont(new Font("Segoe UI", Font.BOLD, 16));
                g2d.drawString("🚗 Car Availability", 20, 30);
    ...
    private JPanel createRecentRentals() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.setOpaque(false);

        // Header
        JLabel header = new JLabel("📋 Recent Rentals");
        ...
        // Rentals list with better layout
        JPanel rentalsList = new JPanel();
        rentalsList.setLayout(new BoxLayout(rentalsList, BoxLayout.Y_AXIS));
        rentalsList.setOpaque(false);

        // Get recent rentals (last 4)
        ...
        for (Rental rental : recentRentals) {
            ...
            JPanel rentalItem = createRentalItem(
            ...
            rentalsList.add(Box.createVerticalStrut(8)); // Consistent spacing
        }

        // If no rentals, show placeholder
        if (recentRentals.isEmpty()) {
            JPanel placeholder = createRentalItem("🚗 No recent rentals", "No data available", "$0", "N/A");
            rentalsList.add(placeholder);
        }

        // Add flexible space at bottom
        rentalsList.add(Box.createVerticalGlue());

        panel.add(rentalsList, BorderLayout.CENTER);
    ...
    private JPanel createRentalItem(String car, String customer, String amount, String time) {
    ...
    private JPanel createStaffAlerts() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.setOpaque(false);

        // Header
        JLabel header = new JLabel("⚠ Staff Alerts");
        header.setFont(new Font("Segoe UI", Font.BOLD, 16));
        header.setForeground(ModernUITheme.TEXT_PRIMARY);
        header.setBorder(BorderFactory.createEmptyBorder(0, 0, 15, 0));
        panel.add(header, BorderLayout.NORTH);
        ...
        List<String[]> alerts = generateStaffAlerts(cars, rentals);
    ...
    private List<String[]> generateStaffAlerts(List<Car> cars, List<Rental> rentals) {
        ...
        if (maintenanceCars > 0) {
            alerts.add(new String[] { "● Medium", maintenanceCars + " car(s) in maintenance", "3 hours ago" });
        }
    ...
    /**
     * Get the current staff member's name for welcome message
     */
    private String getStaffWelcomeMessage() {
    ...
    private void showDashboard() {
        ...
        updateHeader(getStaffWelcomeMessage());
    }
    ...
    private void showCarSearch() {
        cardLayout.show(mainContentPanel, "CAR_SEARCH");
        updateHeader("🔍 Search Cars");
    }
    ...
    private JPanel createStatisticsPanel() {
        JPanel statsPanel = new JPanel(new GridLayout(1, 4, 20, 0)); // Changed from 3 to 4 columns
        ...
        try {
            ...
            // Get current staff ID for individual revenue calculation
            User currentUser = authService.getCurrentUser();
            BigDecimal staffRevenue = BigDecimal.ZERO;
            if (currentUser != null) {
                staffRevenue = rentalService.getRevenueByStaffId(currentUser.getId());
            }

            // Create stat cards
            statsPanel.add(createStatCard("🚗", "Available Cars",
                    String.valueOf(cars.stream().filter(c -> "AVAILABLE".equals(c.getStatus())).count()),
            ...
            statsPanel.add(createStatCard("💰", "My Revenue",
                    "$" + staffRevenue, // Show individual staff revenue
                    ModernUITheme.INFO_COLOR));

        } catch (SQLException e) {
            // If database error, show placeholder stats
            statsPanel.add(createStatCard("🚗", "Available Cars", "0", ModernUITheme.SUCCESS_COLOR));
            ...
            statsPanel.add(createStatCard("💰", "My Revenue", "$0", ModernUITheme.INFO_COLOR));
        }
